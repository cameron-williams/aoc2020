...
/// Day 8 Solutions
/// 
/// Day 8 takes an input of multiple lines with a command on each. The command can either be nop (no operation), acc (add to accumulator), or jmp (jump to instruction).
/// The given instructions will be an infinite loop, day 1 problem is to find the value of acc before the first instance of any instruction being run twice.
fn day8_part1() -> Result<isize, Error> {
...
/// Day 2 problem is finding the value of the accumulator when the program terminates after changing one nop->jmp or jmp->nop which fixes the infinite loop in the program.
fn day8_part2() -> Result<isize, Error> {
    let input = fs::read_to_string("./day8_input.txt")?;
    let instructions: Vec<(&str, isize, usize)> = input.split("\n").filter_map(|l| {
        let mut split = l.split(" ");
        if let (Some(instr), Some(int)) = (split.next(), split.next()) {
...
fn main() -> Result<(), std::io::Error> {
    ...
    let answer = day8_part2();

    println!("Day 8 answer is {:?}", answer);
    Ok(())
}
